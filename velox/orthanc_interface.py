...
class OrthancInterface:
    ...
    def _get_report_ids(self):
        data = {"Level": "Instance", "Query": {"SeriesDescription": "Rho-Report"}}

        headers = {"Content-Type": "application/json"}
        response = requests.post(
            f"{self.DXA_ORTHANC_URL}/tools/find", json=data, headers=headers
        )
    ...
    def _download_report_dicoms(self):
        for report_id in self.report_ids:
            ...
            response = requests.get(
                f"{self.DXA_ORTHANC_URL}/instances/{report_id}/file"
            )
    ...
    def _delete_reports(self):
        for report_id in self.report_ids:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Failed to delete report ({report_id}). Status code: {response.status_code}, Response: {response.text}"
                )
